...
export default function CoverCard({ cover, index, onView, onEdit, onDelete }) {
  ...
  return (
    <article
      className="animate-rise group mb-5 break-inside-avoid cursor-pointer overflow-hidden rounded-xl transition-all duration-200 hover:-translate-y-1 hover:rotate-[0.4deg] hover:shadow-theme-lg"
      style={{ 
    ...
    >
        ...
        <div className="absolute inset-0 flex items-end justify-between bg-gradient-to-t from-black/85 via-black/20 p-3 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
          <div className="flex gap-1.5">
            <button
              ...
              className="rounded-full bg-white/90 p-2 text-symbiote-900 backdrop-blur-sm transition-transform hover:scale-110"
              aria-label="View"
            >
              <IconEye size={16} />
            </button>
          ...
          <button
            ...
            className={`rounded-full p-2 backdrop-blur-sm transition-all ${
              confirmDelete
                ...
                : 'bg-white/90 text-symbiote-900 hover:bg-venom-500 hover:text-white'
            }`}
      ...
      <div className="p-3">
        <h3 className="truncate text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{cover.title}</h3>
        <div className="mt-0.5 flex items-center justify-between">
          <span className="truncate text-xs" style={{ color: 'var(--text-muted)' }}>{cover.artist}</span>
          {cover.year && (
            <span className="ml-2 shrink-0 text-xs tabular-nums" style={{ color: 'var(--text-muted)' }}>{cover.year}</span>
          )}
        </div>
      </div>
    </article>
